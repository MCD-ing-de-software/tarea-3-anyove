...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        # Verificar que el resultado es correcto usando numpy.testing.assert_allclose()
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
        # Verificar que el resultado tiene la forma esperada (3 elementos para window=2) usando self.assertEqual
        expected_shape = (3,)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        utils = StatisticsUtils()
        arr = [1, 2, 3]
        # Verificar que se lanza ValueError para window=0
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=0)
        # Verificar que se lanza ValueError para window mayor que la longitud del array
        with self.assertRaises(ValueError):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        utils = StatisticsUtils()
        arr = [[1, 2], [3, 4]]
        # Verificar que se lanza ValueError para secuencia multidimensional
        with self.assertRaises(ValueError):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        """
        utils = StatisticsUtils()
        # Crear una lista de números para calcular z-scores
        arr = [10.0, 20.0, 30.0, 40.0]
        # Llamar a zscore para obtener los z-scores
        result = utils.zscore(arr)
        # Verificar que la media del resultado es aproximadamente 0 usando self.assertAlmostEqual
        self.assertAlmostEqual(result.mean(), 0.0, places=7)
        # Verificar que la desviación estándar del resultado es aproximadamente 1 usando self.assertAlmostEqual
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        """
        utils = StatisticsUtils()
        # Crear una lista con todos los valores iguales para probar zscore
        arr = [5.0, 5.0, 5.0]
        # Verificar que se lanza ValueError para secuencia con desviación estándar cero
        with self.assertRaises(ValueError):
            utils.zscore(arr)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        """
        utils = StatisticsUtils()
        # Crear una lista de números para probar min_max_scale
        arr = [2.0, 4.0, 6.0]
        # Llamar a min_max_scale para obtener los valores escalados
        result = utils.min_max_scale(arr)
        # Verificar que el valor mínimo del resultado es 0.0 usando self.assertAlmostEqual
        self.assertAlmostEqual(result.min(), 0.0, places=7)
        # Verificar que el valor máximo del resultado es 1.0 usando self.assertAlmostEqual
        self.assertAlmostEqual(result.max(), 1.0, places=7)
        # Verificar que los valores transformados son correctos usando numpy.testing.assert_allclose
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        """
        utils = StatisticsUtils()
        # Crear una lista con todos los valores iguales para probar min_max_scale
        arr = [3.0, 3.0, 3.0]
        # Verificar que se lanza ValueError para secuencia con todos los valores iguales
        with self.assertRaises(ValueError):
